...
export default createEvent({
    ...
    async run(reaction, user) {
        ...
        const message = await user.messages.fetch(reaction.messageId, reaction.channelId)
        const reactionObject = message.reactions?.find(r => r.emoji.id === reaction.emoji.id && r.emoji.name === reaction.emoji.name)
